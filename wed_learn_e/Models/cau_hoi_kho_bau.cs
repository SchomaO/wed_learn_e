//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace wed_learn_e.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class cau_hoi_kho_bau
    {
        public cau_hoi_kho_bau()
        {
            this.lich_su_kho_bau = new HashSet<lich_su_kho_bau>();
        }
    
        public int id_cau_hoi { get; set; }
        public Nullable<int> id_game { get; set; }
        public Nullable<int> thu_tu { get; set; }
        public string loai_cau_hoi { get; set; }
        public string noi_dung { get; set; }
        public string dap_an_a { get; set; }
        public string dap_an_b { get; set; }
        public string dap_an_c { get; set; }
        public string dap_an_d { get; set; }
        public string dap_an_dung { get; set; }
        public string manh_ghep { get; set; }
    
        public virtual game_kho_bau game_kho_bau { get; set; }
        public virtual ICollection<lich_su_kho_bau> lich_su_kho_bau { get; set; }
    }
}
